...
- **사람은 루프 안에 없다** — 자동 실행(PIE) + 로그 지표 + 규칙 기반 판정. 질문은 착수 전(①·②)과 종료 보고에서만(LOOP 규칙 0).
...
## 루프 한 줄
> **①인테이크 → ②DOR게이트 → ③분해 → ④레벨×4(3h씩, 매 티켓 리뷰) → ⑤판정 → (기각/애매면 복귀)**
> 실행: **`/uefn-game-loop`**. 인테이크도 루프의 한 단계다(기각되면 되돌아옴).
> 착수 후에는 멈추지 않는다. 막힌 것은 `state/deferred.md`에 모아 종료 후 한 번에 묻는다. 상세: `loop/LOOP.md` 규칙 0.
...
| **`spec/GENRE-PACKS.md`** | **장르별로 뭘 물을까** — 7개 팩(전투·생존·퍼즐·경쟁·협동·이동·건설)의 결정적 디테일·필수질문·지표·변주축 |
...
| **`spec/UEFN-FEASIBILITY.md`** | **UEFN에서 되나** — 실측 제약표·등급(🟢🟡⚫🔴)·대체 설계 절차 |
| **`spec/DOR.md`** | **착수해도 되나** — 기획서 준비 완료 기준(핵심: 판단 가능성) |
| **`loop/review/ADVERSARIAL.md`** | **판단 가능한가** — 적대적 리뷰어(티켓마다 빠른공격 + 게이트 전면공격) |
| **`loop/TIMEBOX.md`** | **3시간 안에** — 레벨당 예산 배분·초과 규칙·세션 로그 |

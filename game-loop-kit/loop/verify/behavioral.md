...
- 로그에는 지표뿐 아니라 단계 태그(`step=Sn`)와 만남 이벤트(`meet=<역할들> loc=<좌표>`)를 남긴다(`flow.md` §5). 플로우 리포트의 재료다.
- 3회까지 구현 수정 → 재검증. 그래도 실패 → 해당 레벨 `무효` 표시 + 원인 요약을 `state/deferred.md`에 기록 → 다음 레벨(LOOP 규칙 0, 사람 호출 없음).

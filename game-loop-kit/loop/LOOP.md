...
시각 지도: `loop/LOOP-MAP.html`(브라우저로 열기) — 바깥 루프·질문 경계·기획 라운드·티켓 게이트·막힘 처리 분기를 한 장에 그린 것. 문서가 바뀌면 지도도 갱신한다.
...
- 환경 장애(로그아웃·런처·프리즈·MCP 도구 소실)는 `loop/recover.md` 절차로 처리한다. 조기 종료는 하드 실패(같은 복구 절차 2회 연속 실패, 4레벨 타임박스 소진)뿐이며 종료 보고로 끝낸다.

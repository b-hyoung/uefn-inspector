...
### B-6. 역량 주장 공격 ⭐
- "그건 된다/안 된다"의 근거가 **`capabilities` 도구 출력**인가, 문서·추측인가?
  (문서와 도구가 다르면 **도구가 맞다** — 문서를 고쳐라)
...
## 등급 → 처리
- 🔴 **Blocker** — 결론이 틀린다 → 해당 티켓/실험 중단·수정 (측정 문제면 **실험 무효·재실행**). 루프 중 사람 호출 사유는 아니다(LOOP 규칙 0) — 수정 티켓으로 처리하고 종료 보고에 기재.

...
라운드제. 시간 상한 없음, 종료는 조건으로만(종이 질문 0 · 전면공격 3회 연속 Blocker 0 · 미결 카드 0 · 페르소나 15판 · 사용자 확인). 라운드마다 필수 스킬을 **AI가** 선언하고 호출한다(`grilling`·`domain-modeling`·`research`·`brainstorming`·`uefn-review`). 사용자는 이 스킬을 부르지 않는다. 유일하게 사용자에게 묻는 구간이며, 질문은 전부 grilling 형식의 결정 카드다.
**애매함 처리 규칙:** DOR 전에는 애매하면 grilling으로 묻는다. DOR 후에는 묻지 않고 deferred(규칙 0). 종료 보고의 질문도 grilling 라운드 하나로 낸다.

...
- 실측 예: **@editable 배선(최초·변경 모두)🟢**(uefn-inspector `bind_editable`, UEFN 닫고 — 퍼블리시 수용만 미검증) ·
  **디바이스 설정값🟢**(자작 Verse `@editable`에 두면 오프라인 읽기/쓰기; 스톡 디바이스 설정은 ⚫GUI) ·
  새 @editable 슬롯 자체 추가🟡(`add_binding`, UEFN 수용 미검증) · 조명 벽통과🟡 · 조준점 색변경🔴(→오디오/포스트프로세스) · 외부통신🔴
- ⚠️ **라이브로 못 하는 것 ≠ 불가능** — ⚫/🔴 판정 전에 반드시 `capabilities` 호출 → HARNESS §1.5 오프라인 표 확인.
  라이브 거부 메시지("not valid ScriptDevice", 타입 거부)는 **오프라인 몫이라는 신호**지 불가 근거가 아니다.
...
## 정지게이트 (여기서만 사람 호출)
**`capabilities`에도 오프라인 경로가 없는 GUI 전용 작업**(@editable 배선은 이제 `bind_editable`로 오프라인 처리 — 정지 사유 아님) · 행동검증 3회 실패 · 스펙 모호/창작결정 · 파괴적 작업 · 새 외부 의존성
...
## 흔한 함정
자동 구동 모드 부재(급소) · **라이브 실패를 "GUI 전용"으로 오판**(→ `capabilities` 먼저, 배선은 `bind_editable`) · 오프라인 쓰기는 **UEFN 닫고**(파일 잠금) ·

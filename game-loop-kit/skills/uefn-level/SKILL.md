...
- `spec/MAP.md` 구역표·동선을 `build.md` 구조 절에 옮겨 적고 시작한다(뼈대는 이 표를 좌표로 옮기는 작업이다)

## 티켓 4종 · 3시간 배분
| # | 티켓 | 예산 | 통과 기준 |
|---|---|---|---|
| 1 | 레벨 뼈대(공간·배치) | 30분 | `inspect_level` 기대 디바이스·개수 + **구조 설명서**(`level_map` 평면도·구역표·동선·스크린샷 3장, `build.md` 구조 절). 설명서 없으면 `done` 불가. `design_lint` 뼈대 기준(verticality·occupancy·function_elements) fail 0. **플로우 검증 ②**(`loop/verify/flow.md`, 자리를 `level_map` 좌표로, F7 실측) fail 0 |
...
| 3 | 측정 로그 심기 | 30분 | PIE 로그에 지표별 `[M]` 라인 **전부** 출력 + 단계 태그(`step=Sn`)·만남 이벤트(`meet=`) — 없으면 플로우 리포트 불가 |
...
- **구조는 설명할 수 있어야 한다**(규칙 9). 맨 도형을 놓기만 한 레벨은 뼈대 티켓도 통과하지 못한다. `level_map(path)`로 평면도·구역표를 뽑아 `build.md` 구조 절(테마·구역·동선·스크린샷)을 채운다. 남이 그 절만 읽고 재현할 수 없으면 빠른공격에서 Blocker.
...
## 끝나면
- **플로우 리포트** `levels/LEVEL-X/flow-report.md`(`loop/verify/flow.md` §6): 실제 플로우 vs 계획, 만남, 줄별 재미를 로그 근거로 설명. 설명 안 되는 줄 = 재미 공백 → 수정 티켓 → 재실행(2회째도 공백이면 레벨 무효). 이게 없으면 레벨 `done` 아님.

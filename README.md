...
MCP 도구 11개: `capabilities` · `inspect_level` · **`level_map`**(평면도·구역표) · **`design_lint`**(설계 됐는지 중간 점검) · `audit` · `find` · `who_uses` · **`read_actor`**(Verse-VM 값) · **`editable_bindings`**(@editable 배선 읽기) · **`bind_editable`**(@editable 배선 쓰기, 오프라인) · `engine_devices`
...
`capabilities` · `inspect_level` · `level_map`(평면도·구역표·동선 템플릿) · `design_lint`(그레이박스·자산 다양성·높이·점유·회전·광원 체크) · `audit` · `find` · `who_uses` · `read_actor`(Verse-VM 값 포함) · `editable_bindings` · `engine_devices`
